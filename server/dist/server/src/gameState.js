...
class ServerGameState {
    constructor(roomId, mapId, onUpdate) {
    ...
    addPlayer(id, characterType, spawnIndex) {
        const cfg = index_1.CHARACTER_CONFIGS[characterType];
        const map = index_2.MAP_CONFIGS[this.state.mapId];
    ...
    applyInput(playerId, input) {
        ...
        if (!player || player.isDead)
            return;
        const cfg = index_1.CHARACTER_CONFIGS[player.characterType];
        const now = Date.now();
    ...
    resolveAttack(attacker) {
        const cfg = index_1.CHARACTER_CONFIGS[attacker.characterType];
        if (cfg.isRanged) {
            const id = nextProjId();
    ...
    applyDamage(target, attacker, damage, knockback) {
        if (target.isDead)
            return;
    ...
    tick() {
        ...
        for (const player of Object.values(this.state.players)) {
            ...
            if (player.attackCooldown > 0)
                player.attackCooldown -= TICK_RATE;
        ...
        for (const [id, proj] of Object.entries(this.state.projectiles)) {
            ...
            for (const player of Object.values(this.state.players)) {
                ...
                if (dx < cfg.width / 2 + 8 && dy < cfg.height / 2 + 8) {
                    ...
                    const kbForce = attacker ? index_1.CHARACTER_CONFIGS[attacker.characterType].knockbackForce : 200;
                    this.applyDamage(player, { x: proj.x, y: proj.y }, proj.damage, kbForce);
                    delete this.state.projectiles[id];
                    break;
                }
            }
